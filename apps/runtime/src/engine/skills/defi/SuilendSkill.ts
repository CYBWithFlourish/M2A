...
import { createSuiClient, suilendPackageId, SUI_CLOCK } from '../../../config.js';

const client = createSuiClient();

const SUILEND_ACTIONS = ['lend', 'borrow', 'getPosition'] as const;

export const suilendSkill: SkillDefinition = {
  id: 'suilend-lending',
  ...
  requiredServices: ['suilend', 'sui-rpc'],
  requiresFunds: true,
  inputSchema: {
    type: 'object',
    properties: {
      action: { type: 'string', enum: SUILEND_ACTIONS, description: 'Lending action' },
  ...
  async execute(params: SkillParams, context: ExecutionContext): Promise<SkillResult> {
    try {
      ...
      if (!SUILEND_ACTIONS.includes(action as typeof SUILEND_ACTIONS[number])) {
